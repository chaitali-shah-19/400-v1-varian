...
%                                     fmirror_LRpos = fmirror(1).moveAbsolute(-21347); %Absolute Coil Pos [data]
%                                     fmirror_UDpos = fmirror(2).moveAbsolute(27614); %Absolute Coil Pos [data]
...
%                                 disp('Wrote Sage write 4: Move down and read the data in');
                                ...
                                t.StopFcn =  @(~,~) stop(t);
                                t.StartDelay = 600+30; %180s exp+30+T1delay_time %100=70s exp +30s wait
                                %t.StartDelay =10; %100
                                t.ExecutionMode = 'singleshot';
                                start(t);
                                wait(t);
% 
%                                 fmirror_LRpos = fmirror(1).moveAbsolute(-21698); %Absolute Hyperpolarize Pos [data]
%                                 fmirror_UDpos = fmirror(2).moveAbsolute(27965); %Absolute Hyperpolarize Pos [data]
...
%                                 fprintf('Laser mirror UD moved to hyperpolarize pos:\n %.2f.\n', fmirror(2).getPosition());
                ...
                t2.StopFcn =  @(~,~) stop(t2);
                t2.StartDelay = 130+600; %shuttle time is 130s for velocity 5 plus additional 600s wait time to accomodate temperature stabilization of 5 min experiments
