...
remoteAddr = '192.168.0.181'; % new computer
remotePort = 202
localPort = 9090;
    ...
    while ( connect==on )
        ...
        switch cmdByte
            ...
            case 3 % Setup Receive
                ...
                res = inst.SendScpi('DIG:TRIG:DEL:EXT 5e-06'); %12e-06 %5.5e-06
                assert(res.ErrCode == 0);
            ...
            case 4 % MAIN READOUT CODE
                ...
                twahdead=2+3+10+4+tacq; %twahdead=rof2+rof1+4+neco*dtacq
                time_cycle=pw*4+(6*twahdead)*1e-6; %time for one wahuha cycle [us]
                ...
                pulseAmpWAH=pulseAmp(6:6:end);
                ivecWAH=1:length(pulseAmpWAH);
                time_axisWAH=time_cycle*ivecWAH;
                % Save data
                try
                    figure(1);clf;
                    plot(time_axisWAH,pulseAmpWAH,'r-');
                    xlabel('Time [s]');
                    ylabel('Total Signal [a.u.]');
                    set(gca,'ylim',[0 max(pulseAmpWAH)*1.1]);
                    figure(12);clf;
                    plot(time_axis,relPhase); %not fixed for WAHUHA sequence
                    xlabel('Time [s]');
